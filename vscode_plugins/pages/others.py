...
@rx.page(
    route=Route.OTHERS.value,
    title=utils.other_title,
    description=utils.other_description,
    image=utils.preview,
    meta=utils.other_meta,
    on_load=PageState.plugin_links("Other")
)
def others() -> rx.Component:
    return rx.box(
        utils.lang(),
        navbar(showInit=True),
        heading_gallery("Plugins de otros tipos"),
        rx.box(
            rx.cond(
